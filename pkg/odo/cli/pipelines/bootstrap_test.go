...
Checking if OpenShift Pipelines Operator is installed with the default configuration`
	...
	buff := &bytes.Buffer{}
	fakeSpinner := &mockSpinner{writer: buff}
	bootstrapParams := &BootstrapParameters{&pipelines.BootstrapOptions{}, &genericclioptions.Context{}}
	err := checkBootstrapDependencies(bootstrapParams, fakeClient, fakeSpinner)

	assertError(t, err, "")
	if bootstrapParams.SealedSecretsService.Name != "sealed-secrets-controller" && bootstrapParams.SealedSecretsService.Namespace != "kube-system" {
